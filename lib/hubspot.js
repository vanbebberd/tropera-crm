...
// Genera semanas entre start y end (más reciente primero)
function weeksInRange(start, end) {
  ...
  while (d.getTime() <= end) {
    ...
    const sun = new Date(d); sun.setDate(sun.getDate() + 6); sun.setHours(23, 59, 59, 999);
    weeks.push({
      start: mon.getTime(),
      end:   sun.getTime(),
      label: mon.toLocaleDateString('es-CL', { day: '2-digit', month: '2-digit' }),
...
async function getDealStages() {
  ...
  (data.results || []).forEach(p => {
    ...
    byPipeline[key] = { pipelineId: p.id, closedWonIds: closed, visitadoIds: visitado };
  });

  _stageCache = { ...byPipeline, all: { pipelineId: null, closedWonIds: allClosed, visitadoIds: allVisitado } };
  return _stageCache;
}

module.exports = { get, search, dateFilter, getOwners, weekRange, weeksInRange, getDealStages };
