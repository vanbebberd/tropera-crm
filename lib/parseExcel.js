...
function parseVentasExcel(buffer) {
  ...
  const resultado = Object.values(vendedores).map(v => {
    ...
    // Velocidad por cliente: semanas entre compras consecutivas
    const semanasPorCliente = {};
    v.clientesPorSemana.forEach((semanaSet, weekIdx) => {
      semanaSet.forEach(cliente => {
        if (!semanasPorCliente[cliente]) semanasPorCliente[cliente] = [];
        semanasPorCliente[cliente].push(weekIdx);
      });
    });

    // Top 10 clientes por litros totales
    const clientesTop = Object.entries(v.litrosPorCliente)
      .map(([nombre, litros]) => ({ nombre, litros: Math.round(litros * 10) / 10 }))
      .sort((a, b) => b.litros - a.litros)
      .slice(0, 10);

    // Clientes en riesgo: compraron la semana anterior pero no la más reciente
    const nSem = v.clientesPorSemana.length;
    const semReciente  = v.clientesPorSemana[nSem - 1] || new Set();
    const semAnterior  = v.clientesPorSemana[nSem - 2] || new Set();
    const clientesEnRiesgo = [...semAnterior]
      .filter(c => !semReciente.has(c))
      .map(c => ({ nombre: c, litros: Math.round((v.litrosPorCliente[c] || 0) * 10) / 10 }))
      .sort((a, b) => b.litros - a.litros);

    const clientesVelocidad = Object.entries(semanasPorCliente).map(([nombre, semanas]) => {
      let velocidad = null;
      if (semanas.length >= 2) {
        const gaps = [];
        for (let i = 1; i < semanas.length; i++) gaps.push(semanas[i] - semanas[i - 1]);
        velocidad = Math.round((gaps.reduce((a, b) => a + b, 0) / gaps.length) * 10) / 10;
      }
      return { nombre, apariciones: semanas.length, velocidad };
    }).sort((a, b) => {
      // primero los con velocidad (más frecuentes primero), luego los de una sola compra
