...
export default function VentasSection({ data, onRefresh, ownerFiltro, ownerNombre }) {
  ...
  const [velVendedor, setVelVendedor] = useState('');
    ...
    <div className="space-y-6">
      ...
      {sinMatchExcel ? (
        ...
        <>
          ...
          <VelocidadClientesTable
            todosVendedores={todosVendedores}
            velVendedor={velVendedor}
            setVelVendedor={setVelVendedor}
            colors={COLORS}
          />
        </>
      )}
...
function VelocidadClientesTable({ todosVendedores, velVendedor, setVelVendedor, colors }) {
  const [busqueda,      setBusqueda]      = useState('');
  const [formatoFiltro, setFormatoFiltro] = useState('todos');

  const vendedores = todosVendedores || [];
  const tieneVelocidad = vendedores.some(v => v.clientesVelocidad?.length > 0);
  const nombresV = vendedores.map(v => v.nombre);

  const formatos = (() => {
    const s = new Set();
    vendedores.forEach(v => (v.clientesVelocidad || []).forEach(c => { if (c.formato) s.add(c.formato); }));
    return [...s].sort();
  })();
  ...
  vendedores.forEach((v, vi) => {
    if (velVendedor && v.nombre !== velVendedor) return;
    (v.clientesVelocidad || []).forEach(c => {
      if (formatoFiltro !== 'todos' && c.formato !== formatoFiltro) return;
      if (q && !(c.nombre || '').toLowerCase().includes(q) && !(v.nombre || '').toLowerCase().includes(q)) return;
      filas.push({ ...c, vendedor: v.nombre, colorIdx: vi });
    });
  });
  filas.sort((a, b) => {
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {tieneVelocidad && formatos.length > 0 && (
            <div className="flex items-center bg-gray-800 border border-gray-700 rounded-lg p-0.5 gap-0.5">
              <button onClick={() => setFormatoFiltro('todos')}
        ...
        {tieneVelocidad && (
          <div className="flex items-center gap-2">
            {vendedores.length > 1 && (
              <select
                value={velVendedor}
                onChange={e => setVelVendedor(e.target.value)}
              ...
              >
                ...
                {vendedores.map(v => <option key={v.nombre} value={v.nombre}>{shortName(v.nombre, nombresV)}</option>)}
              </select>
            )}
            <input
              value={busqueda}
              ...
              className="bg-gray-800 border border-gray-700 text-white text-xs rounded-lg px-3 py-1.5 w-40 focus:outline-none focus:border-orange-500 placeholder-gray-600"
            />
          </div>
        )}
      </div>
        ...
        {!tieneVelocidad ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filas.map(f => (
                  <tr key={`${f.vendedor}-${f.nombre}-${f.formato}`} className="border-b border-gray-800/40 hover:bg-gray-800/20">
                    <td className="px-4 py-2">
                      <span className="text-xs font-medium" style={{ color: colors[f.colorIdx % colors.length] }}>
                        {shortName(f.vendedor, nombresV)}
                      </span>
                    </td>
