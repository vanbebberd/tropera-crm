...
// Cache de stage IDs por probabilidad (se resuelve una vez por proceso)
...
async function getDealStages() {
  ...
  (data.results || []).forEach(p => {
    (p.stages || []).forEach(s => {
      ...
      if (prob >= 0.99)              closedWonIds.push(s.id);
      else if (prob >= 0.3 && prob < 0.99) visitadoIds.push(s.id);
    });
  });
  _stageCache = { closedWonIds, visitadoIds };
  return _stageCache;
}
