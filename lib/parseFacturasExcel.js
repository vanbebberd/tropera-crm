...
function parseFacturasExcel(buffer) {
  ...
  const ALIAS = {
    nombre:    ['nombre', 'cliente', 'razon social', 'empresa', 'razon', 'vendedor'],
    recaudado: ['recaudado', 'cobrado', 'pagado', 'facturado'],
    vencido:   ['vencido', 'vencida', 'vencidos', 'mora', 'deuda vencida'],
    abierto:   ['abierto', 'abierta', 'pendiente', 'por cobrar', 'corriente'],
    va:        ['v/a', 'v / a', 'va', 'variacion', 'indice'],
  };
  ...
  for (let i = 0; i < Math.min(rows.length, 30); i++) {
    ...
    rows[i].forEach((raw, j) => {
      ...
      for (const [key, aliases] of Object.entries(ALIAS)) {
        ...
        if (aliases.some(a => cell === norm(a) || cell.includes(norm(a)))) {
          found[key] = j;
        }
      }
    });
